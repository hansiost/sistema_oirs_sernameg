...
import { ArrowLeft, FileDown, Calendar as CalendarIcon, FilterX, Clock, BarChart3 } from 'lucide-react';
...
const generateMockData = (): TiempoRespuesta[] => {
  return REGIONES_CHILE.map(region => {
    ...
    return {
      ...
      cantidadSolicitudes: Math.floor(Math.random() * 150) + 50,
    };
  });
};
...
const chartConfig = {
  ...
  queja: { label: 'Queja', color: 'hsl(var(--chart-3))' },
  sugerencia: { label: 'Sugerencia', color: 'hsl(var(--chart-4))' },
  felicitacion: { label: 'Felicitación', color: 'hsl(var(--chart-5))' },
} satisfies ChartConfig;


const TiemposRespuestaBarChart = ({ data }: { data: TiempoRespuesta[] }) => {
    return (
        <Card>
            <CardHeader>
                <CardTitle className="flex items-center gap-2">
                    <BarChart3 className="h-5 w-5" />
                    Tiempos de Respuesta Promedio por Región y Tipo
                </CardTitle>
                <CardDescription>Comparación de los días de respuesta promedio para cada tipo de solicitud entre regiones.</CardDescription>
            </CardHeader>
            <CardContent>
                <ChartContainer config={chartConfig} className="min-h-[500px] w-full">
                    <BarChart data={data}>
                        ...
                        <Bar dataKey="reclamo" fill="var(--color-reclamo)" radius={4} />
                        <Bar dataKey="queja" fill="var(--color-queja)" radius={4} />
                        <Bar dataKey="sugerencia" fill="var(--color-sugerencia)" radius={4} />
                        <Bar dataKey="felicitacion" fill="var(--color-felicitacion)" radius={4} />
                    </BarChart>
                </ChartContainer>
            </CardContent>
        </Card>
...
const TiemposRespuestaReport = () => {
    ...
    const promediosNacionales = useMemo(() => {
        ...
        if (total === 0) return { consulta: 0, reclamo: 0, queja: 0, sugerencia: 0, felicitacion: 0, promedioGeneral: 0, cantidadSolicitudes: 0 };
        const sum = data.reduce((acc, item) => ({
            ...
            cantidadSolicitudes: acc.cantidadSolicitudes + item.cantidadSolicitudes,
        }), { consulta: 0, reclamo: 0, queja: 0, sugerencia: 0, felicitacion: 0, promedioGeneral: 0, cantidadSolicitudes: 0 });
        ...
        return {
            ...
            cantidadSolicitudes: sum.cantidadSolicitudes,
        };
    }, [data]);
    ...
    return (
        <div className="space-y-6">
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow>
                                    ...
                                    <TableHead className="text-right font-bold">Cant. Solicitudes</TableHead>
                            ...
                            {data.map((item) => (
                                <TableRow key={item.region}>
                                    ...
                                    <TableCell className="text-right">{item.cantidadSolicitudes}</TableCell>
                            ...
                            <TableFooter>
                                <TableRow className="bg-muted/50 hover:bg-muted/80">
                                    <TableHead className="font-bold">Promedio / Total Nacional</TableHead>
                                    <TableHead className="text-right font-bold">{promediosNacionales.cantidadSolicitudes}</TableHead>
            ...
            <div className="mt-6">
                <TiemposRespuestaBarChart data={data} />
            </div>
        </div>
    );
};
